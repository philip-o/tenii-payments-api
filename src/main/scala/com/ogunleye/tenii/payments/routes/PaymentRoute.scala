...
class PaymentRoute(implicit system: ActorSystem, breaker: CircuitBreaker) extends Directives with LazyLogging {
  ...
  def route: Route = pathPrefix("payment") {
    makeTeniiPayment
  }

  def makeTeniiPayment: Route =
    post {
      entity(as[TeniiPotCreditRequest]) { request =>
        logger.info(s"POST /payment - $request")
        onCompleteWithBreaker(breaker)(paymentActor ? request) {
          case Success(msg: TeniiPotCreditResponse) if msg.cause.isEmpty => complete(StatusCodes.Created -> msg)
